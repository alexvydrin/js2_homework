...
const app = new Vue({
    ...
    data: {
        ...
        showCart: false,  // По умолчанию корзина не показана
        ...
        cartItems: [], // Массив для списка продуктов в корзине
        filtered: [], // Массив для списка продуктов на сайте после использования фильтра
        imgCart: 'img/product.png', // Картинка для продукта в корзине
        products: [],  // Массив для списка продуктов на сайте (каталог товаров)
        imgProduct: 'img/product.png', // Картинка для продукта в каталоге
        error: false
    },

    imgProduct: 'https://placehold.it/200x150', // Картинка для продукта в каталоге
    ...
    methods: {
        ...
        getJson(url) {
            ...
            return fetch(url)
                ...
                .catch(error => this.error = true) // Неудача
        },
                ...
                .then(data => {
                    if (data.result === 1) { // Есть разрешение
                        ...
                        if (find) { // Такой продукт уже есть в корзине
                            find.quantity++; // Увеличиваем количество в корзине на единицу
                ...
                .then(data => {
                    if (data.result === 1) { // Есть разрешение
                        if (item.quantity > 1) { // Такой продукт еще есть в корзине и больше 1
                            item.quantity--; // Уменьшаем количество в корзине на единицу
        ...
        filter() {
            ...
            // Накладываем фильтр на исходный полный список продуктов
        ...
        this.getJson(`${API + this.catalogUrl}`)
            .then(data => {
                for (let item of data) {
                    ...
                    this.$data.filtered.push(item); // Первоначально отфильтрованный список равен полному списку
                }
            });
    }
});
